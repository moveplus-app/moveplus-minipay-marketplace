...
class _AddProductDialogState extends State<AddProductDialog> {
  ...
  Future<void> _registerProduct() async {
    ...
    try {
      ...
      // Sanitize filename: remove invalid characters and replace spaces/special chars
      // Invalid characters for storage keys: < > : " | ? * and apostrophe
      String sanitizedFileName = fileName;
      // Replace invalid characters one by one to avoid regex issues
      ...
      sanitizedFileName = sanitizedFileName.replaceAll('?', '_');
      sanitizedFileName = sanitizedFileName.replaceAll('*', '_');
      sanitizedFileName = sanitizedFileName.replaceAll("'", '_'); // Apostrophe
      sanitizedFileName = sanitizedFileName.replaceAll(' ', '_'); // Spaces
      // Replace any remaining non-alphanumeric characters (except - and _ and .)
      sanitizedFileName = sanitizedFileName.replaceAll(RegExp(r'[^\w\-_.]'), '_');
      
      // Create storage path with timestamp and sanitized filename
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final imagePath = 'products/${timestamp}_$sanitizedFileName';
      
      String? imageUrl;
      try {
        // Try marketplace_images bucket first
        ...
        // If marketplace_images bucket doesn't exist, try post_images as fallback
      ...
      await supabaseService.createMarketplaceItem(
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Product "${_titleController.text}" added successfully!'),
      ...
      if (mounted) {
        ...
        if (errorMessage.contains('quota exceeded') || errorMessage.contains('Storage quota')) {
          errorMessage = '❌ Storage quota exceeded! Supabase free tier has 1 GB limit. Go to Supabase Dashboard → Storage to check usage and delete old files.';
        } else if (errorMessage.contains('RLS policy') || errorMessage.contains('row-level security')) {
          errorMessage = '❌ Storage RLS policy blocked upload. Check storage policies for "marketplace_images" bucket.';
        } else if (errorMessage.contains('Bucket not found') || errorMessage.contains('404')) {
          errorMessage = '❌ Storage bucket "marketplace_images" not found. Please create it in Supabase Storage dashboard.';
        } else if (errorMessage.contains('StorageException') || errorMessage.contains('Storage error')) {
          errorMessage = '❌ Storage error. Possible causes:\n• Storage quota exceeded (1 GB free tier limit)\n• RLS policy blocking upload\n• Bucket not found\n\nCheck Supabase Dashboard → Storage for details.';
