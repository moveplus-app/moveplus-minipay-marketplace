...
-- ---------------------------------------------------------------------------
-- History types: allow reservation lifecycle markers
-- ---------------------------------------------------------------------------
ALTER TABLE public.energy_points_history
  DROP CONSTRAINT IF EXISTS energy_points_history_transaction_type_check;

ALTER TABLE public.energy_points_history
  ADD CONSTRAINT energy_points_history_transaction_type_check
  CHECK (
    transaction_type IN (
      'earned',
      'redeemed',
